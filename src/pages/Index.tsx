    ...
    <Layout>
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-center space-y-8">
          ...
          <div className="flex flex-wrap justify-center gap-3 animate-fade-in" style={{ animationDelay: "0.3s" }}>
            {buttons.map((btn) => (
              <Button
                ...
                size="lg"
                variant="outline"
                className="text-base px-6 py-5 font-heading font-bold gap-2 border-primary text-primary hover:bg-primary hover:text-primary-foreground"
                onClick={() => {
        ...
        {features && features.length > 0 && (
          <div className="w-full max-w-3xl space-y-6 animate-fade-in" style={{ animationDelay: "0.4s" }}>
            <h2 className="font-heading text-2xl font-bold text-primary">مزايا السيرفر</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {features.map((f) => (
                <Card key={f.id} className="bg-card border-border hover:border-primary transition-colors">
